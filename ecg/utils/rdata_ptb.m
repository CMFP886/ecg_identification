...
% 计算阈值
threshold = (max_threshold - min_threshold) * 0.5;

% 初步标注 R 波波峰
[r_peaks_row, r_peaks_col] = find(six_coeffs > threshold);
...
for i = 1:length(r_peaks_col)-1
    ...
    if R_peak_intervals(i) < 0.3 * mean_R_interval
        %去除幅度较小的一个 R 波
        [~, min_index] = min(six_coeffs(r_peaks_col(i:i+1)));
